...
from tests.fixtures import create_account, main_window  # noqa
import os
...
def test_create_account_with_same_account_name(qtbot, create_account, monkeypatch):
    ...
    assert (
        MockWarningDialog._called_text
        == "Account with name TestAccount is already exist. Try different name"
    )


@pytest.mark.parametrize("symbol", r'[\\/:*?"<>|]')
def test_create_account_with_forbidden_symbols(
    qtbot, symbol, create_account, monkeypatch
):
    ...
    assert (
        MockWarningDialog._called_text
        == rf'Used a forbidden symbol {symbol} at position 4. Do not use [\\/:*?"<>|[\]] characters'
    )
...
def test_create_account_with_too_long_name(qtbot, create_account, monkeypatch):
    ...
    assert (
        MockWarningDialog._called_text
        == "The name is too long, use less than 60 characters"
    )
