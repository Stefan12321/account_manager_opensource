...
class prepare_import_file:
    def __init__(self, account_name: str, file_data: str, settings_file_name: str):
        self.zip_path = fr"{os.environ['ACCOUNT_MANAGER_BASE_DIR']}\export_test.zip"
        self.test_account_path = fr"{os.environ['ACCOUNT_MANAGER_BASE_DIR']}\{account_name}"
    ...
    def __enter__(self):
        ...
        with open(fr"{os.environ['ACCOUNT_MANAGER_BASE_DIR']}\{self.account_name}\{self.settings_file_name}", "w") as f:
            f.write(self.file_data)

        with zipfile.ZipFile(self.zip_path, mode='w') as zipf:
            zipf.write(fr"{os.environ['ACCOUNT_MANAGER_BASE_DIR']}\{self.account_name}\{self.settings_file_name}",
                       fr"{self.account_name}\{self.settings_file_name}")
...
def test_import_profile(qtbot: pytestqt.qtbot.QtBot, create_account, monkeypatch):
    ...
    monkeypatch.setattr("app.components.browser_tabs.QtWidgets.QFileDialog", MockQFileDialog)
    ...
    with prepare_import_file("TestAccount",
